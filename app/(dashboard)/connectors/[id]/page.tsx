...
export default async function ConnectorDetailPage({
  ...
  const { data: row } = await supabase
    .from("connectors")
    .select("id, kind, name, status, config")
    .eq("id", id)
    .maybeSingle();

  if (!row || row.kind !== "woocommerce") {
    notFound();
  }
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Consumer key/secret never appear in logs; they are encrypted at rest.</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Posts a synthetic signed payload to <code className="text-xs">/api/webhooks/woocommerce/{row.id}</code>.
          </CardDescription>
        </CardHeader>
