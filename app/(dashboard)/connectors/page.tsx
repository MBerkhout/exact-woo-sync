      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Exact Online uses OAuth and registers webhooks on connect; WooCommerce lands in Phase 2.
          </p>
        </div>
        {role === "admin" ? <AddExactConnectorDialog /> : null}
      </div>
                ...
                connectors!.map((c) => (
                  <TableRow key={c.id}>
                    <TableCell className="font-medium">
                      <Link href={`/connectors/${c.id}`} className="hover:underline">
                        {c.name}
                      </Link>
                    </TableCell>
